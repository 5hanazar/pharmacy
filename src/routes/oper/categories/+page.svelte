<script lang="ts">
	...
	const destroy = async (id: number) => {
		const response = await fetch(`${base}/oper/categories/${id}`, {
			method: "DELETE",
			headers: {
	...
	const move = async () => {
		...
		if (browser) await goto(`${base}/oper/categories?${arr.join("&")}`, { replaceState: true });
		loading = false;
	};
	...
	<DataTable {headers} bind:sortKey bind:sortDirection rows={data.data} sortable>
		...
		<Toolbar>
			<ToolbarContent>
				...
				<Button icon={Add} kind="ghost" href="{base}/oper/categories/new">Create</Button>
			</ToolbarContent>
		</Toolbar>
	</DataTable>
