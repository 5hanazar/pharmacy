<script lang="ts">
	...
	const destroy = async (id: number) => {
		const response = await fetch(`/oper/categories/${id}`, {
			method: "DELETE",
			headers: {
	...
	const move = async () => {
		...
		if (browser) await goto(`/oper/categories?${arr.join("&")}`, { replaceState: true });
		loading = false;
	};
	...
	<DataTable {headers} bind:sortKey bind:sortDirection rows={data.data} sortable>
		<svelte:fragment slot="cell" let:row let:cell>
			{#if cell.key == "overflow"}
				<OverflowMenu flipped>
					<OverflowMenuItem href="/oper/categories/{row.id}" text="Edit" />
		...
		<Toolbar>
			<ToolbarContent>
				...
				<Button icon={Add} kind="ghost" href="/oper/categories/new">Create</Button>
			</ToolbarContent>
		</Toolbar>
	</DataTable>
