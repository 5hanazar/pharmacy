<script lang="ts">
	...
	const destroy = async (id: number) => {
		const response = await fetch(`${base}/oper/clients/${id}`, {
			method: "DELETE",
			headers: {
	...
	const headers = [
		...
		{ key: "username", value: "Имя", sort: _ab0 },
		{ key: "phone", value: "Телефон", sort: _ab0 },
		{ key: "name", value: "Полное имя", sort: _ab0 },
		{ key: "description", value: "Описание", sort: _ab0 },
		{ key: "address", value: "Адрес", sort: _ab0 },
		{ key: "devices", value: "Устройства", sort: false },
		{ key: "hashJwt", value: "Hash JWT", sort: false },
		{ key: "createdDate", value: "Дата создания", sort: false },
		{ key: "onlineDate", value: "Дата посещения", sort: false },
	...
	const move = async () => {
		...
		if (browser) await goto(`${base}/oper/clients?${arr.join("&")}`, { replaceState: true });
		loading = false;
	};
	...
	<DataTable {headers} bind:sortKey bind:sortDirection rows={data.data} sortable>
		<svelte:fragment slot="cell" let:row let:cell>
			{#if cell.key == "overflow"}
				<OverflowMenu flipped>
					<OverflowMenuItem href={`${base}/oper/clients/${row.id}`} text="Изменить" />
		...
		<Toolbar>
			<ToolbarContent>
				...
				<Button icon={Add} kind="ghost" href={`${base}/oper/clients/new`}>Создать</Button>
			</ToolbarContent>
		</Toolbar>
	</DataTable>
