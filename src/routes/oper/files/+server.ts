...
export async function GET({ url }) {
	...
	const result: FileDto[] = files.map(function (fileName) {
		const e = fs.statSync(`${filePathUploads}/${fileName}`)
		return {
			id: fileName,
...
export async function POST({ request }) {
	...
	fs.writeFileSync(`${filePathUploads}/${file.name}`, new DataView(buffer))
...
export async function PUT({ request }) {
	...
	return new Response(null, {
		status: (await shell(`sudo unzip ${filePathUploads}/${fileName} -d ${filePathUploads}`))
	});
}
export async function DELETE({ request }) {
	const fileName = await request.text();
	const e = fs.statSync(`${filePathUploads}/${fileName}`)
	if (e.isDirectory()) fs.rmSync(`${filePathUploads}/${fileName}`, { recursive: true });
	else fs.unlinkSync(`${filePathUploads}/${fileName}`)
	return new Response(null, {
		status: 200,
	});
}
